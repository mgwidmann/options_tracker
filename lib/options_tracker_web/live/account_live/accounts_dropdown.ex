defmodule OptionsTrackerWeb.AccountLive.AccountsDropdown do
  ...
  @spec update(%{account_path_fun: (any, any, any -> any), all_path_fun: (any, any -> any)}, Phoenix.LiveView.Socket.t()) :: {:ok, Phoenix.LiveView.Socket.t()}
  def update(%{all_path_fun: all_path_fun, account_path_fun: account_path_fun} = assigns, socket) when is_function(all_path_fun, 2) and is_function(account_path_fun, 3) do
    current_account =
  ...
  def handle_event("change_account", %{"account_id" => "all"}, socket) do
    {:noreply,
     socket
     |> push_redirect(to: socket.assigns.all_path_fun.(socket, :index))}
  end
  ...
  def handle_event("change_account", %{"account_id" => account_id}, socket) do
    ...
    {:noreply,
     socket
     |> push_redirect(to: socket.assigns.account_path_fun.(socket, :index, account_id))}
  end
end
